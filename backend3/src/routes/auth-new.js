 ...
 * Returns status info to determine next step in auth flow
 */
router.post('/verify-alumni', async (req, res) => {
  try {
    ...
    // Determine user status for auth flow
    const hasTemporaryPassword = Boolean(user.temporaryPassword);
    const hasPermanentPassword = Boolean(user.passwordHash && user.passwordHash !== 'google_oauth');
    
    let status = 'unknown';
    if (hasTemporaryPassword && !hasPermanentPassword) {
      status = 'needs_temp_verification'; // First-time setup, admin created account
    } else if (hasPermanentPassword) {
      status = 'has_password'; // Already set up, redirect to login
    }

    console.log(`📊 [VERIFY-ALUMNI] User status - hasTemp: ${hasTemporaryPassword}, hasPerm: ${hasPermanentPassword}, status: ${status}`);
    ...
    return res.status(200).json({
      ...
      status, // new field: 'needs_temp_verification' | 'has_password' | 'unknown'
...
router.post('/verify-temp-password', async (req, res) => {
  try {
    ...
    // Mark user as first login (still needs to set permanent password)
