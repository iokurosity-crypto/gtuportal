...
router.get('/', async (req, _res) => {
    ...
    try {
        // Build filter - if userId is provided, filter by authorId
        const filter = userId ? { authorId: userId } : {};
        console.log('🏆 [CHALLENGES] GET / - userId:', userId, 'filter:', filter);
        
        const challenges = await Challenge_1.default.find(filter)
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limit)
            .populate('authorId');
        ...
        const total = await Challenge_1.default.countDocuments(filter);
