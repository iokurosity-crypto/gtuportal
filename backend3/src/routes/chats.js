...
router.post('/create', requireAuth, async (req, res) => {
  try {
    ...
    const existingChat = await ChatRoom.findOne({
      ...
      .populate('user1', 'name email department batch profilePhoto profilePicture photoURL currentJobTitle currentCompany')
      .populate('user2', 'name email department batch profilePhoto profilePicture photoURL currentJobTitle currentCompany');
    ...
    await newChat.save();
    await newChat.populate('user1', 'name email department batch profilePhoto profilePicture photoURL currentJobTitle currentCompany');
    await newChat.populate('user2', 'name email department batch profilePhoto profilePicture photoURL currentJobTitle currentCompany');
...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    const chats = await ChatRoom.find({
      ...
      .populate({
        path: 'lastMessage',
        populate: { path: 'senderId', select: 'name email profilePhoto profilePicture photoURL' },
      })
      .sort({ updatedAt: -1 });
...
router.get('/:chatId/messages', requireAuth, async (req, res) => {
  try {
    ...
    const messages = await Message.find({ chatId })
      .populate('senderId', 'name email profilePhoto profilePicture photoURL')
      .sort({ createdAt: 1 });
...
router.post('/:chatId/messages', requireAuth, async (req, res) => {
  try {
    ...
    await message.save();
    await message.populate('senderId', 'name email profilePhoto profilePicture photoURL');
...
router.get('/:userId/:partnerId/last-message', requireAuth, async (req, res) => {
  try {
    ...
    const chat = await ChatRoom.findOne({
      ...
      path: 'lastMessage',
      populate: { path: 'senderId', select: 'name profilePhoto profilePicture photoURL' },
    });
