...
const ensureDb = (res) => {
    if (mongoose_1.default.connection.readyState !== 1) {
        ...
        res.status(503).json({ message: 'Database temporarily unavailable. Please try again in a few seconds.' });
        return false;
    }
    return true;
};
...
router.get('/', async (req, res) => {
    ...
    try {
        // Build filter - if userId is provided, filter by organizerId
        const filter = userId ? { organizerId: userId } : {};
        console.log('📍 [EVENTS] GET / - userId:', userId, 'filter:', filter);
