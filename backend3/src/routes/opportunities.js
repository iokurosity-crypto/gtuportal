...
// Returns all internships/jobs with pagination or filters by userId if provided
router.get('/', async (req, res) => {
  try {
    ...
    // Build filter - if userId is provided, filter by postedById
    const filter = userId ? { postedById: userId } : {};
    console.log('💼 [OPPORTUNITIES] GET / - userId:', userId, 'filter:', filter);
    
    const opportunities = await Internship.find(filter)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
